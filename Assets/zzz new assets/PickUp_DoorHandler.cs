...
public class PickUp_DoorHandler : MonoBehaviour
{
    ...
    [HideInInspector] public bool _hasAccess;
    [HideInInspector] public GameObject _access;
    [HideInInspector] public GameObject _otherGA;
    [HideInInspector] public bool _canPressforPick;
    [HideInInspector] public bool _canPressforOpne;
    
    public otthers _others;


    public AudioSource _audioSource;
    public AudioClip _doorOpne,_doorClose,_keyPick,_keyDrop;
    ...
    void PickUP()
    {
        ...
        if(_otherGA != null )
        {
            ...
            //{
                ...
                _otherGA = null;
                _audioSource.clip = _keyPick;
                _audioSource.Play();
                Debug.Log("you have picked the wrong key");
            //}
            //else if(_hasAccess)
            //{
    ...
    void Door_Open()
    {
        if(_otherGA != null)
        {
            if (_access.name == "Key")
            {
                ...
                //if(_otherGA.transform.GetChild(0))
                //{
                //    _otherGA.transform.GetChild(0).gameObject.SetActive(false);
                //}
                _otherGA.gameObject.SetActive(false);
                _canPressforOpne = true;
                _hasAccess = false;
                _audioSource.clip = _doorOpne;
                _audioSource.Play();
            ...
            else
            {
                ...
                _infoText.text = "You have picked the wrong key".ToString();
                _infoText.gameObject.SetActive(true);
                _audioSource.clip = _keyDrop;
                _audioSource.Play();
                Invoke("Tewxt_False", 1f);
