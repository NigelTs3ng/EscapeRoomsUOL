    ...
    public void InteractWithObject(GameObject currentInteractable, ref bool isHoldingItem, ref GameObject heldItem)
    {
        if (currentInteractable != null)
        {
            ...
            if (currentInteractable.CompareTag("Key"))
            {
                ...
                {
                    Debug.LogError(currentInteractable.name);
                    if(currentInteractable.tag == "Key")
                    {
                        //if (currentInteractable.name == "Key")
                        //{
                        //    Debug.LogError("Right Key");
                            //PlayerController._instance._keyNumber = 1;
                            PlayerController._instance._hasKey = true;
                        //}
                        //else if (currentInteractable.name != "Key")
                        //{
                        //    Debug.LogError("Wroung Key");
                        //    PlayerController._instance._keyNumber = 2;
